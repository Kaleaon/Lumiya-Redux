...
A class that no source reproduces yet ships as its **original compiled code**:

* `tools/recover/bytecode_classes.txt` lists these classes (226 today).
* `app/build.gradle` stops compiling their `.java` and links
  `app/libs/lumiya-original-classes.jar` instead.
...
* `app/stable-ids.txt` pins all 3,798 original resource ids (`aapt2
  --stable-ids`), so ids compiled into the original bytecode still resolve.

The `.java` files of these classes stay in the tree as the work-in-progress
recovery. To move a class back to source, fix its `.java`, delete its line from
...
| | classes |
| --- | --- |
| verified identical in behaviour (`OK`) | 1,232 |
| only additions (javac/jadx artefacts, `sdk_folded` branches) | 191 |
| reviewed differences in `accepted.txt` | 25 |
| **damaged** | **0** |
| shipped as original bytecode | 226 |

## Decisions left for the maintainer

* **Behaviour from earlier passes that original bytecode replaced.** Earlier
  ...
  files did not verify, they now run the original code. The 92 affected
  classes are listed in `docs/recovery/superseded_hand_edits.txt`. To bring a
  fix back, apply it on top of faithful source and record it in
  `accepted.txt`.

...
REMAINING_JAVA_FILE = ROOT / "docs/modernization/remaining-java.txt"
...
def remaining_java_report(rows: list[dict[str, str]]) -> str:
    sources = [row["source"] for row in rows if row["language"] == "java"]
    header = [
        "# Remaining production Java sources",
        "# Generated by: python3 tools/modernization/inventory.py --remaining-java",
        f"# Count: {len(sources)}",
        "",
    ]
    return "\n".join(header + sources) + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--remaining-java",
        action="store_true",
        help="emit every production Java source awaiting migration",
    )
    ...
    if args.check:
        ...
        expected_report = remaining_java_report(rows)
        if not REMAINING_JAVA_FILE.is_file():
            errors.append(f"remaining-Java report does not exist: {REMAINING_JAVA_FILE}")
        elif REMAINING_JAVA_FILE.read_text(encoding="utf-8") != expected_report:
            errors.append(
                "remaining-Java report is stale; regenerate it with "
                "python3 tools/modernization/inventory.py --remaining-java > "
                "docs/modernization/remaining-java.txt"
            )
        if errors:
            print("\n".join(errors), file=sys.stderr)
            return 1
    if args.remaining_java:
        print(remaining_java_report(rows), end="")
        return 0
    if args.csv:
